...
Collins 在讲完设备谱系之后，专门留了一段给数学。欧洲数学在 1530 年代（Cardano 与 Tartaglia 之争）到 1630 年代（Descartes）之间发生过一次快速发现的起飞，引擎不是经验观察，而是符号机器：从言语论证与缩写，转向标准化符号与操作方程组的规则。这套数学「机械」一旦发明，研究就像物理科学里改装设备一样滚动起来——一代代符号被创造、重组，每一步都产出新结果。数学凭自己领域内部的动力成为快速发现科学，并不靠伺候自然科学。

这一段有一个精确的当代对应物，大模型实验室天天都在演示。LLM 加形式化验证（Lean 一类证明助手）构成一套新的符号机器：模型批量生成候选证明，验证器例行化复核——恰好是 Collins 说的「改装到现象能被随意复现」。第五节那道验收门槛，在这间房间里过得干干净净：AlphaTensor 改进矩阵乘法算法，FunSearch 推进集合论的上界，AlphaProof 与 AlphaGeometry2 拿到 IMO 银牌级成绩——都是真现象，不是吹的。数学因此成了整栋大楼里唯一一间模型通过验收的房间：现象可复现，共识可自动化，检查器就是这一代人的 Boyle 气泵。

所以各实验室不约而同拿数学炫技，结构上完全说得通：不是巧合，是筛选——只有这里，快速发现与例行化同时成立。但 Collins 在那段话的结尾埋了一根刺，值得原样记住：纯数学的技术与发现链条「按自己领域内部的动力推进」，而纯数学的发展「并没有中心性地决定自然科学中经验发现的过程」。符号机器的起飞自成一界，可以几百年不欠经验世界的债。翻译过来就是：模型在数学里的快速发现，不能自动兑现为其他领域的经验发现——而后者才是「引力」真正声称要组织的东西。实验室拿数学当样板间，演示的是全楼最好的房间；第一节里那 95% 试点无损益影响的注脚，才是其余房间交的房租。样板间是真的，房价是另一回事。
...
- AI for Math 案例：AlphaTensor（*Nature*, 2022）；FunSearch（*Nature*, 2023）；AlphaProof + AlphaGeometry2，IMO 2024 银牌级（DeepMind）
