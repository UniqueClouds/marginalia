---
id: marginalia-014
title: "扫描书高保真夹心 OCR——把普特南和罗蒂变成'原版一模一样+全文可检索'的 PDF"
date: 2026-08-22
published: 2026-08-22
kind: analysis(工程复盘)
sources:
  - "普特南《理性、真理与历史》(童世骏/李光程 译,上海译文)。z-library 扫描版:314 页,283DPI 灰度 JPEG,61MB,无文本层"
  - "罗蒂《哲学和自然之镜》(李幼蒸 译,商务印书馆)。z-library 扫描版:521 页,CCITT G4 二值,19.9MB,无文本层"
  - "RapidOCR (rapidocr_onnxruntime 1.2.3,PP-OCRv3 det/rec ONNX 模型);onnxruntime-directml 1.24(DmlExecutionProvider)"
  - "PyMuPDF 1.28(pymupdf):TextWriter/render_mode=3、get_pixmap、xref 流操作;PIL Pillow 12(PNG optimize/group4)"
  - "本仓库 PR #36:完整可复用管线 ocr_pipeline/(build_ocr_pdf.py / build_compressed.py / verify_output.py)"
initial-prompt: "选用合适的方案给这两本书的 pdf 进行 ocr,目标导出是一个图片质量高保真压缩,文字可精准选中、复制粘贴的,原版内容一模一样的 pdf 文件。"
agent: ZCode CLI
model: ox-alpha
issue: 37
---
